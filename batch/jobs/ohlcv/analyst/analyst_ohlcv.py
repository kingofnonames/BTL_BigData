...
#     closes, volumes, daily_returns, cum_returns, gains, losses = [], [], [], [], [], []
...
#         ma30 = sum(closes[-30:])/min(len(closes),30)
#         std30 = (sum([(c-ma30)**2 for c in closes[-30:]])/min(len(closes),30))**0.5
...
#         losses.append(loss)
#         if i<13:
#             rsi14 = 100*sum(gains)/max(sum(gains)+sum(losses),1e-8)
#         else:
